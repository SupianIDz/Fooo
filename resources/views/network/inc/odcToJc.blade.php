<div class="w-full">
    <template x-for="(row, index) in cableLinesWithODCPorts" :key="index">
        <div class="w-full bg-white shadow-lg p-2 px-4 mb-3 border last:mb-0 rounded-lg hover:shadow">
            <div class="grid grid-cols-2 items-center ">
                <div class="flex items-center space-x-3">
                    <div x-html="'<span class=\'text-indigo-500 font-bold\'>' + row.attached.name + '</span> ' + row.name"></div>
                </div>
            ...
            <div class="mt-2 p-5 rounded-lg border w-full" x-show="row.show">
                <div class="flex justify-end mb-3 space-x-3">
                    <button type="button" x-on:click="addOutputPortODC(row)"
